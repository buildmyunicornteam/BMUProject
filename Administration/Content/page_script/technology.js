...
$(document).ready(() => {
    $("#StartupGrid").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: {
                    url: "/Technology/GetAllTechnologyList"
        ...
        columns: [{
            template: '<button class="btn btn-info" title="Edit Technology" onclick=Edit("#: ID #")><i class="icon-pencil"></i></button>\
                      <button class= "btn btn-danger" title="Delete Technology" onclick=Delete("#: ID #") > <i class="fa fa-trash"></i></button> ',
            width: 100
        },
...
$("#editStartupForm").submit(function (e) {
    ...
    $.ajax({
        url: GetBaseURL() + "Technology/EditTechnology",
        method: "POST",
        ...
        success: function (response) {
            if (response == "OK") {
                ...
                swal("Success!", "Technology Updated Successfully", "success");
...
$("#newStartupForm").submit(function (e) {
    ...
    $.ajax({
        url: GetBaseURL() + "Technology/AddTechnology",
        method: "POST",
...
function Edit(Value) {
    ...
    $.ajax({
        url: GetBaseURL() + "Technology/GetTechnology/",
        method: "GET",
        data: { ID: Value },
        dataType: 'json',
